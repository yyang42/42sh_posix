...
static bool			builtin_popd_error(char *text, char *msg)
{
	shenv_singl_error(1, "popd: %s: %s", text, msg);
	argparser_print_help(builtin_popd_argparser());
	return (false);
}
...
static bool			init_flags(t_builtin_dirs *this)
{
	...
	while ((token = twl_lst_get(this->token_copy, index)))
	{
		if (*token->text == '-' && token->text[1] == '-' && !token->text[2])
			...
			return (builtin_popd_error(token->text, "invalid number"));
		else
			return (builtin_popd_error(token->text, "does not take arguments"));
	}
	return (true);
}
