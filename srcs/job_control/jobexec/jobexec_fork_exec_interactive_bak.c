...
static void			job_utils_waitpid_wrapper(t_lst *all_tokens, pid_t pid,
					void (wait_fn)(int pid, void *exec_ctx), void *exec_ctx)
{
	t_job           *job;
...
void				jobexec_fork_exec_interactive_bak(t_lst *all_tokens, void *exec_ctx,
					...
					void (execve_fn)(void *ctx))
{
	...
	else
	{
	    ...
	    else
	    {
			...
			job_utils_waitpid_wrapper(all_tokens, pid, wait_fn, exec_ctx);
