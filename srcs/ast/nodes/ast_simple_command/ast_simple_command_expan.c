...
static void 	iter_assign_fn(void *assign_, void *should_exec_)
{
	...
	should_exec = should_exec_;
	assign = assign_;
	*should_exec = expan_init(&assign->key, assign->key_unexpanded, TOKEN_ORIGIN_ASSIGNMENT_KEY);
	*should_exec = expan_init(&assign->value, assign->value_unexpanded, TOKEN_ORIGIN_ASSIGNMENT_VALUE);
}
...
static void 	iter_redir_fn(void *redir_, void *should_exec_)
{
	...
	should_exec = should_exec_;
	redir = redir_;
	*should_exec = expan_init(&redir->param, redir->param_unexpanded, TOKEN_ORIGIN_REDIR_PARAM);
	*should_exec = expan_init(&redir->heredoc_text, redir->heredoc_text_unexpanded, TOKEN_ORIGIN_REDIR_HEREDOC);
}
