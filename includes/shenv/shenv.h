...
void						shenv_shflag_set(t_shenv *this, char mono,
		bool enabled);
...
char						*shenv_concat_pos_param_char(t_shenv *env,
		char *sep);
...
int							shenv_print_error_printf(t_shenv *this,
		int line, char *fmt, ...);
void						shenv_singl_error(int exit_code,
		char *fmt, ...);
void						shenv_singl_error_simple(int exit_code,
		char *fmt, ...);
...
void						shenv_set_binary_saved_path(t_shenv *env,
		char *path);
int							shenv_set_read_buffer_ptr(t_shenv *env, int fd);
void						shenv_clear_stdin_read_buffer(t_shenv *env);
void						shenv_execve(t_shenv *env, char *path,
		t_lst *argv_lst);
void						shenv_execve_findpath(t_shenv *env,
		t_lst *argv_lst);
...
void						shenv_set_current_directory(t_shenv *this,
		char *fm_whom);
void						shenv_reset_current_directory(t_shenv *this,
		char *new_dir);
char						*shenv_get_current_directory(t_shenv *this,
		char *fm_whom);
...
void						shenv_add_shell_func(t_shenv *env, char *key,
		struct s_ast_function_def *data);
